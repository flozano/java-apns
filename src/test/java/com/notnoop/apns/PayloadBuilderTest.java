...
public class PayloadBuilderTest {
	...
	public void localizedOneWithArray() {
		final PayloadBuilder builder = new PayloadBuilder().localizedKey(
				"GAME_PLAY_REQUEST_FORMAT").localizedArguments(
				new String[] { "Jenna", "Frank" });
		builder.sound("chime");
	...
	public void localizedTwo() {
		final PayloadBuilder builder = new PayloadBuilder().sound("chime")
	...
	public void abitComplicated() {
		...
		builder.sound("chime");
		builder.category("NEW_CATEGORY");
		builder.localizedKey("GAME_PLAY_REQUEST_FORMAT").localizedArguments(
				new String[] { "Jenna", "Frank" });

		final String expected = "{\"achme\":\"foo\",\"aps\":{\"sound\":\"chime\",\"category\":\"NEW_CATEGORY\",\"alert\":{\"loc-key\":\"GAME_PLAY_REQUEST_FORMAT\",\"loc-args\":[\"Jenna\",\"Frank\"]}}}";
		final String actual = builder.toString();
		assertEqualsJson(expected, actual);
	}
	...
	public void abitComplicatedEnglishLength() {
		...
		final SimpleApnsNotification fromString = new SimpleApnsNotification(
				deviceToken, builder.build());
		final SimpleApnsNotification fromBytes = new SimpleApnsNotification(
				dtBytes, Utilities.toUTF8Bytes(builder.build()));

		final String expected = "{\"aps\":{\"alert\":\"test\"}}";
		final int actualPacketLength = 1 + 2 + dtBytes.length + 2
				+ /* payload length = */Utilities.toUTF8Bytes(expected).length;
	...
	public void detectingLongMessages() {
		...
		final int cutoffForAlert = Utilities.MAX_PAYLOAD_LENGTH
				- wrapperOverhead;
		...
		assertTrue(border.length() == Utilities.MAX_PAYLOAD_LENGTH);
	...
	public void shrinkLongMessages() {
		...
		final int cutoffForAlert = Utilities.MAX_PAYLOAD_LENGTH
				- wrapperOverhead;
		final int max_length = Utilities.MAX_PAYLOAD_LENGTH;
	...
	public void shrinkLongMessagesWithOtherthigns() {
		...
		final PayloadBuilder bitShort = payloadOf(cutoffForAlert - 1).sound(
				"default");
		bitShort.shrinkBody(); // NOOP
		assertFalse(bitShort.isTooLong());
		...
		final PayloadBuilder abitLong = payloadOf(cutoffForAlert + 1).sound(
				"default");
		abitLong.shrinkBody();
		assertFalse(abitLong.isTooLong());
		assertTrue(abitLong.length() == max_length);

		final PayloadBuilder tooLong = payloadOf(cutoffForAlert + 1000).sound(
				"default");
		tooLong.shrinkBody();
		assertFalse(tooLong.isTooLong());
		assertTrue(tooLong.length() == max_length);
	...
	public void removeAlertIfSooLong() {
		final PayloadBuilder tooLong = APNS.newPayload()
				.customField("test", strOfLen(Utilities.MAX_PAYLOAD_LENGTH))
				.alertBody("what");
		tooLong.shrinkBody();
	...
	private void assertEqualsJson(final String expected, final String actual) {
		...
		try {
			...
			final Map<String, Object> exNode = mapper.readValue(expected,
					Map.class), acNode = mapper.readValue(actual, Map.class);
	...
	public void trimWorksWithLongFields() {
		final PayloadBuilder builder = new PayloadBuilder();
		final String toolong = "1234567890123456789012345678901234567890"
